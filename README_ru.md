...
### Как это работает
Приложение использует root-доступ для изменения базы данных конфигурации PVR (`/data/user_de/0/com.pvr.configuration/databases/config.db`). Оно изменяет параметры `sdk_eyebuffer`, `sdk_enableFFRBySYS`, `sdk_stencilMeshStatus` и `sdk_EyeTextureFov` в таблице `RuleBean`.
